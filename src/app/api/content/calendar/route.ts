/**
 * API Route: Content Calendar
 * GET /api/content/calendar?year=2026&month=4
 * Returns content-specific calendar data: writing, review, publish, meetings, deadlines
 */
...
export const GET = withAuth(async (req, { user }) => {
  try {
    ...
    const [meetings, events, tasks] = await Promise.all([
      // Meetings related to content/creative
      ...
      // Events (writing, review, publish, deadline)
      prisma.event.findMany({
        where: {
          ...
          type: { in: ['WRITING', 'REVIEW', 'PUBLISH', 'MEETING', 'DEADLINE'] },
        },
        orderBy: { date: 'asc' },
        take: 50,
      }),

      // Tasks with content-related keywords
          ...
          OR: [
            ...
            { creatorId: user.id },
            {
              OR: [
                ...
                { title: { contains: 'article', mode: 'insensitive' } },
    ...
    // Transform data into content calendar events
    const eventsList = [
      ...
      ...events.map(event => ({
        id: event.id,
        title: event.title,
        type: event.type as 'WRITING' | 'REVIEW' | 'PUBLISH' | 'MEETING' | 'DEADLINE',
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        writing: eventsList.filter(e => e.type === 'WRITING').length,
        reviews: eventsList.filter(e => e.type === 'REVIEW').length,
        publishes: eventsList.filter(e => e.type === 'PUBLISH').length,
...
// POST /api/content/calendar — Create content event
export const POST = withAuth(async (req, { user }) => {
  try {
    ...
    const validTypes = ['WRITING', 'REVIEW', 'PUBLISH', 'MEETING', 'DEADLINE']
    if (!validTypes.includes(type)) {
      return NextResponse.json(
        { error: 'Invalid type. Must be WRITING, REVIEW, PUBLISH, MEETING, or DEADLINE' },
        { status: 400 }
      )
    }
