/**
 * API Route: Operations Calendar Data
 * GET /api/operations/calendar?year=2026&month=3
 *
 * Returns operations-specific calendar data: tasks, deployments, milestones,
 * meetings, and events for a specific month
 */
...
export const GET = withAuth(async (req, { user }) => {
  try {
    ...
    const cacheKey = `operations-calendar:${user.id}:${year}-${month}:${isManager ? 'mgr' : 'emp'}`
    ...
    const [meetings, events, tasks, deployments, milestones] = await Promise.all([
      ...
      // Deployments - tasks with type DEPLOYMENT
      prisma.task.findMany({
        where: {
          dueDate: {
            gte: startOfMonth,
    ...
    console.error('Failed to fetch operations calendar data:', error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to fetch operations calendar data' },
      { status: 500 }
    )
  }
})
...
// ============================================
// POST /api/operations/calendar — Create an operations calendar event
// ============================================

const createOperationsItemSchema = z.object({
  title: z.string().min(1, 'Title is required').max(200),
  description: z.string().optional(),
  date: z.string().datetime({ message: 'Valid ISO date required' }),
  endDate: z.string().datetime().optional(),
  type: z.enum(['MEETING', 'TASK', 'DEPLOYMENT', 'REVIEW', 'MILESTONE']),
...
export const POST = withAuth(async (req, { user }) => {
  try {
    ...
    const parsed = createOperationsItemSchema.safeParse(body)
    ...
    if (type === 'MEETING' || (participantIds && participantIds.length > 0)) {
      ...
      result = { ...deployment, itemType: 'deployment' }
      ...
      result = { ...milestone, itemType: 'milestone' }
    } else {
      // Default - create an Event record
      ...
      // If participants were specified, notify them
    ...
    const cachePattern = `operations-calendar:*:${eventDate.getFullYear()}-${eventDate.getMonth() + 1}:*`
    ...
    console.error('Failed to create operations calendar item:', error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to create operations calendar item' },
      { status: 500 }
    )
  }
})
...
// ============================================
// DELETE /api/operations/calendar?id=xxx&type=event|meeting|task
// ============================================
...
export const DELETE = withAuth(async (req, { user }) => {
  try {
    ...
    if (itemType === 'meeting') {
    ...
    } else if (itemType === 'task' || itemType === 'deployment' || itemType === 'milestone') {
      ...
      return NextResponse.json(
        { error: 'Invalid type. Must be "event", "meeting", "task", "deployment", or "milestone"' },
        { status: 400 }
      )
    }

    // Best-effort cache invalidation
    await cache.delPattern('operations-calendar:*').catch(() => {})

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Failed to delete operations calendar item:', error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to delete operations calendar item' },
      { status: 500 }
    )
  }
})
