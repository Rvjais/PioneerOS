/**
 * API Route: Finance Calendar Data
 * GET /api/finance/calendar?year=2026&month=3
 *
 * Returns finance-specific calendar data: invoices, payments, expenses,
 * meetings, and tasks for a specific month
 */
...
export const GET = withAuth(async (req, { user }) => {
  try {
    ...
    const isManager = ['SUPER_ADMIN', 'MANAGER', 'FINANCE_HEAD'].includes(user.role)
    const department = user.department

    // Cache key based on user, role, and month
    const cacheKey = `finance-calendar:${user.id}:${year}-${month}:${isManager ? 'mgr' : 'emp'}`
    ...
    const [meetings, events, tasks, invoices, payments, expenses] = await Promise.all([
      ...
      // Invoices (limited to 100 per month)
    ...
    console.error('Failed to fetch finance calendar data:', error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to fetch finance calendar data' },
      { status: 500 }
    )
  }
})
...
// ============================================
// POST /api/finance/calendar — Create a finance calendar event
// ============================================

const createFinanceItemSchema = z.object({
  title: z.string().min(1, 'Title is required').max(200),
  description: z.string().optional(),
  date: z.string().datetime({ message: 'Valid ISO date required' }),
  endDate: z.string().datetime().optional(),
  type: z.enum(['MEETING', 'INVOICE', 'PAYMENT', 'EXPENSE', 'REVIEW']),
...
export const POST = withAuth(async (req, { user }) => {
  try {
    ...
    if (type === 'MEETING' || (participantIds && participantIds.length > 0)) {
      ...
      // INVOICE, PAYMENT, EXPENSE — create an Event record
    ...
    const cachePattern = `finance-calendar:*:${eventDate.getFullYear()}-${eventDate.getMonth() + 1}:*`
    ...
    console.error('Failed to create finance calendar item:', error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to create finance calendar item' },
      { status: 500 }
    )
  }
})
...
// ============================================
// DELETE /api/finance/calendar?id=xxx&type=event|meeting|task
// ============================================
...
export const DELETE = withAuth(async (req, { user }) => {
  try {
    ...
    const isAdmin = ['SUPER_ADMIN', 'MANAGER', 'FINANCE_HEAD'].includes(user.role)
    ...
    await cache.delPattern('finance-calendar:*').catch(() => {})

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Failed to delete finance calendar item:', error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to delete finance calendar item' },
      { status: 500 }
    )
  }
})
