/**
 * API Route: Analytics Calendar
 * GET /api/analytics/calendar?year=2026&month=4
 * Returns analytics-specific calendar data: reports, reviews, meetings, deadlines
 */
...
export const GET = withAuth(async (req, { user }) => {
  try {
    ...
    const [meetings, events, tasks] = await Promise.all([
      // Meetings related to analytics/reports
      prisma.meeting.findMany({
        where: {
          ...
          OR: [
            ...
            { title: { contains: 'metrics', mode: 'insensitive' } },
          ],
        },
        include: {
      ...
      // Events (reports, reviews, deadlines)
      prisma.event.findMany({
        where: {
          ...
          type: { in: ['REPORT', 'REVIEW', 'MEETING', 'DEADLINE'] },
        },
        orderBy: { date: 'asc' },
        take: 50,
      }),

      // Tasks with analytics-related keywords
          ...
          OR: [
            ...
            { creatorId: user.id },
            {
              OR: [
                ...
                { title: { contains: 'metric', mode: 'insensitive' } },
    ...
    // Transform data into analytics calendar events
    const eventsList = [
      ...
      ...events.map(event => ({
        id: event.id,
        title: event.title,
        type: event.type as 'REPORT' | 'REVIEW' | 'MEETING' | 'DEADLINE',
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        reports: eventsList.filter(e => e.type === 'REPORT').length,
    ...
    console.error('Failed to fetch analytics calendar data:', error)
    return NextResponse.json(
      { error: 'Failed to fetch analytics calendar data' },
      { status: 500 }
    )
  }
})

// POST /api/analytics/calendar — Create analytics event
export const POST = withAuth(async (req, { user }) => {
  try {
    ...
    const validTypes = ['REPORT', 'REVIEW', 'MEETING', 'DEADLINE']
    if (!validTypes.includes(type)) {
      return NextResponse.json(
        { error: 'Invalid type. Must be REPORT, REVIEW, MEETING, or DEADLINE' },
        { status: 400 }
      )
    }
    ...
    } else if (type === 'DEADLINE' || type === 'REPORT') {
    ...
    console.error('Failed to create analytics calendar event:', error)
    return NextResponse.json(
      { error: 'Failed to create analytics calendar event' },
      { status: 500 }
    )
  }
})