...
export default function WebDepartmentPage() {
  ...
  const fetchDepartmentData = async () => {
    try {
      setLoading(true)
      const res = await fetch('/api/manager/departments/web')
      if (res.ok) {
  ...
  const getProjectTypeColor = (type: string) => {
    switch (type) {
      case 'NEW_WEBSITE': return 'bg-purple-500/20 text-purple-400'
      case 'REDESIGN': return 'bg-blue-500/20 text-blue-400'
      case 'MAINTENANCE': return 'bg-slate-800/50 text-slate-200'
      case 'LANDING_PAGE': return 'bg-green-500/20 text-green-400'
  ...
  const getAvailabilityColor = (status: string) => {
    switch (status) {
      ...
      case 'ON_LEAVE': return 'bg-red-500/20 text-red-400'
      default: return 'bg-slate-800/50 text-slate-200'
    }
  }
  ...
  if (!data) {
    return (
      <div className="text-center py-12">
        ...
        <button onClick={fetchDepartmentData} className="mt-4 px-4 py-2 bg-indigo-600 text-white rounded-lg">
          Retry
        </button>
      </div>
    )
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-500 to-violet-500 rounded-xl p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Web Development Department</h1>
            <p className="text-indigo-100">Website Design, Development & Maintenance</p>
          </div>
      ...
      {stats.onLeaveCount > 0 && (
        <div className="bg-red-500/10 border border-red-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <p className="font-medium text-red-800">Capacity Alert: {stats.onLeaveCount} team member{stats.onLeaveCount > 1 ? 's' : ''} on leave</p>
            <p className="text-sm text-red-400">{stats.delayedProjects} projects impacted. Consider reassignment.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-5 gap-4">
        ...
        <div className="glass-card rounded-xl border border-white/10 p-4">
          ...
          <p className="text-3xl font-bold text-indigo-600">{stats.avgQCScore}%</p>
        </div>
        <div className="glass-card rounded-xl border border-white/10 p-4">
          <p className="text-sm text-slate-400">On Leave</p>
          <p className="text-3xl font-bold text-red-400">{stats.onLeaveCount}</p>
        </div>
        <div className="glass-card rounded-xl border border-white/10 p-4">
          <p className="text-sm text-slate-400">Delayed Projects</p>
          <p className="text-3xl font-bold text-amber-400">{stats.delayedProjects}</p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card rounded-xl border border-white/10 overflow-hidden">
          <div className="p-4 border-b border-white/10 bg-slate-900/40">
            <h2 className="font-semibold text-white">Team Availability</h2>
          </div>
                ...
                <div key={member.id} className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex gap-2">
                      <span className={`px-2 py-1 text-xs font-medium rounded ${getAvailabilityColor(member.availability)}`}>
                        {member.availability.replace(/_/g, ' ')}
                      </span>
                    </div>
                  </div>
                  <div className="grid grid-cols-4 gap-2 text-sm">
                    ...
                    <div className="text-center p-2 bg-indigo-500/10 rounded">
                      <p className="font-bold text-indigo-600">{member.projectsActive}</p>
                      <p className="text-xs text-slate-400">Projects</p>
                    </div>
                    <div className="text-center p-2 bg-blue-500/10 rounded">
                      <p className="font-bold text-blue-400">{member.qcScore}%</p>
      ...
      <div className="bg-indigo-500/10 rounded-xl border border-indigo-500/20 p-4">
        <h3 className="font-semibold text-indigo-300 mb-3">Department Overview</h3>
        <div className="grid md:grid-cols-2 gap-4 text-sm text-indigo-200/80">
          <div>
            ...
            <ul className="space-y-1">
              ...
              <li>- On Leave: {stats.onLeaveCount} members</li>
            </ul>
          </div>
          <div>
            <p className="font-medium mb-1">Project Status</p>
            <ul className="space-y-1">
              <li>- On Track: {projects.filter(p => p.status === 'ON_TRACK').length} projects</li>
              <li>- Delayed: {projects.filter(p => p.status === 'DELAYED').length} projects</li>
              <li>- At Risk: {projects.filter(p => p.status === 'AT_RISK').length} projects</li>
            </ul>
          </div>
        </div>
      </div>
