...
export default function SocialDepartmentPage() {
  ...
  const fetchDepartmentData = async () => {
    try {
      setLoading(true)
      const res = await fetch('/api/manager/departments/social')
      if (res.ok) {
  ...
  if (!data) {
    return (
      <div className="text-center py-12">
        ...
        <button onClick={fetchDepartmentData} className="mt-4 px-4 py-2 bg-pink-600 text-white rounded-lg">
          Retry
        </button>
      </div>
    )
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-pink-500 to-rose-500 rounded-xl p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Social Media Department</h1>
            <p className="text-pink-100">Content Creation & Community Management</p>
          </div>
      ...
      <div className="grid grid-cols-5 gap-4">
        ...
        <div className="glass-card rounded-xl border border-white/10 p-4">
          ...
          <p className="text-3xl font-bold text-pink-600">{stats.avgQCScore}%</p>
        </div>
        <div className="glass-card rounded-xl border border-white/10 p-4">
          <p className="text-sm text-slate-400">On Leave</p>
          <p className="text-3xl font-bold text-purple-400">{stats.onLeaveCount}</p>
        </div>
        <div className="glass-card rounded-xl border border-white/10 p-4">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
                ...
                <div key={member.id} className="p-4">
                  ...
                  <div className="grid grid-cols-4 gap-2 text-sm">
                    ...
                    <div className="text-center p-2 bg-pink-50 rounded">
                      <p className="font-bold text-pink-600">{member.projectsActive}</p>
        ...
        <div className="glass-card rounded-xl border border-white/10 overflow-hidden">
          <div className="p-4 border-b border-white/10 bg-slate-900/40">
            <h2 className="font-semibold text-white">Client Accounts</h2>
          </div>
          <div className="divide-y divide-white/10">
            {projects.length === 0 ? (
              <div className="p-8 text-center text-slate-400">
                No active accounts in this department
              </div>
            ) : (
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      ...
                      <div className="flex gap-1 mt-1">
                        <span className="px-2 py-0.5 text-xs bg-slate-800/50 text-slate-300 rounded">Instagram</span>
                        <span className="px-2 py-0.5 text-xs bg-slate-800/50 text-slate-300 rounded">Facebook</span>
                      </div>
                    </div>
                    <span className={`px-2 py-1 text-xs font-medium rounded ${
      ...
      {/* Content Calendar Alert */}
      <div className="bg-pink-50 rounded-xl border border-pink-200 p-4">
        <h3 className="font-semibold text-pink-800 mb-3">Content Status</h3>
        <div className="grid md:grid-cols-3 gap-4 text-sm text-pink-700">
          ...
          <div>
            ...
            <ul className="space-y-1">
              <li>- On Track: {projects.filter(p => p.status === 'ON_TRACK').length} accounts</li>
              <li>- Delayed: {projects.filter(p => p.status === 'DELAYED').length} accounts</li>
              <li>- At Risk: {projects.filter(p => p.status === 'AT_RISK').length} accounts</li>
            </ul>
          </div>
          <div>
            <p className="font-medium mb-1">Action Items</p>
            <ul className="space-y-1">
              <li>- Review delayed content calendars</li>
              <li>- Reassign at-risk accounts</li>
            </ul>
          </div>
        </div>
      </div>
