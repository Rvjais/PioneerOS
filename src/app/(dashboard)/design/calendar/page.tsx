  ...
  return (
    <div className="space-y-6 ">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Design Calendar</h1>
          <p className="text-sm text-slate-500">Track design projects, reviews, approvals, and deliveries</p>
        </div>
        <Link
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        {/* Calendar */}
        <div className="lg:col-span-2 glass-card rounded-xl border border-slate-200 overflow-hidden">
          {/* Month Navigation */}
          <div className="p-4 border-b border-slate-200 bg-gradient-to-r from-purple-600 to-violet-600 text-white">
            <div className="flex items-center justify-between">
          ...
          <div className="grid grid-cols-7 border-b border-slate-200">
            {DAYS.map(day => (
              <div key={day} className="p-3 text-center text-sm font-semibold text-slate-600 bg-slate-100">
                {day}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7">
            ...
            {Array.from({ length: startingDay }).map((_, i) => (
              <div key={`empty-${i}`} className="h-24 border-b border-r border-slate-100 bg-slate-100" />
            ))}
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  className={`h-24 border-b border-r border-slate-100 p-2 text-left hover:bg-violet-50 transition-colors ${
                ...
                >
                  <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-sm font-medium ${
                    isToday(day) ? 'bg-violet-500 text-white' : 'text-slate-700'
                  }`}>
                    {day}
                  </span>
                  <div className="mt-1 space-y-1">
                    ...
                    {dayEvents.length > 2 && (
                      <div className="text-xs text-slate-500 px-1">+{dayEvents.length - 2} more</div>
                    )}
                  </div>
                </button>
              )
      ...
      <div className="glass-card rounded-xl border border-slate-200 p-4">
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-slate-700">Approval</span>
          </div>
          <div className="flex items-center gap-2">
