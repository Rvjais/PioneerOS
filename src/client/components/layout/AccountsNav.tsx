...
const accountsNavigation = [
  ...
  },
  {
    category: 'Clients',
    items: [
      { name: 'Clients', href: '/accounts/clients', icon: 'clients' },
      { name: 'Client Lifecycle', href: '/accounts/client-lifecycle', icon: 'lifecycle' },
      { name: 'Client Guidelines', href: '/accounts/client-guidelines', icon: 'guidelines' },
      { name: 'Comm. Charter', href: '/accounts/communication-charter', icon: 'communication' },
      { name: 'Email Templates', href: '/accounts/email-templates', icon: 'email' },
      { name: 'Discrepancies', href: '/accounts/discrepancies', icon: 'discrepancy' },
    ]
  },
  {
    category: 'Finance',
    items: [
      { name: 'Finance Overview', href: '/accounts/finance-overview', icon: 'finance' },
      { name: 'Invoices', href: '/accounts/invoices', icon: 'invoices' },
      { name: 'Auto Invoice', href: '/accounts/auto-invoice', icon: 'autoInvoice' },
      { name: 'Proforma Invoice', href: '/accounts/proforma-invoice', icon: 'invoices' },
      { name: 'Accounts', href: '/accounts/accounts-view', icon: 'accounts' },
      { name: 'Bank Statements', href: '/accounts/bank-statements', icon: 'bank' },
      { name: 'Ledger', href: '/accounts/ledger', icon: 'ledger' },
      { name: 'Payment Tracker', href: '/accounts/payment-tracker', icon: 'paymentTracker' },
      { name: 'Pending Payments', href: '/accounts/pending-payments', icon: 'pendingPayments' },
      ...
      { name: 'ROI Analysis', href: '/accounts/roi', icon: 'roi' },
    ]
  },
  {
    category: 'Operations',
  ...
  },
  {
    ...
    items: [
      { name: 'Performance', href: '/accounts/performance', icon: 'performance' },
      { name: 'Leaderboard', href: '/accounts/leaderboard', icon: 'leaderboard' },
      { name: 'My Goals', href: '/accounts/my-goals', icon: 'goals' },
      { name: 'Achievements', href: '/accounts/achievements', icon: 'achievements' },
    ]
  },
  {
    category: 'Culture',
...
const icons: Record<string, ReactNode> = {
  ...
  guidelines: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>,
  ...
  discrepancy: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>,
  ...
  onboarding: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>,
  ...
  autoInvoice: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>,
  bank: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 6l9-4 9 4v2H3V6zm0 4h18v10a2 2 0 01-2 2H5a2 2 0 01-2-2V10zm4 3v4m4-4v4m4-4v4" /></svg>,
  ledger: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>,
  paymentTracker: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>,
  pendingPayments: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
  roi: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z" /></svg>,
  ...
  handovers: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>,
  monthly: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>,
  ...
  deliverables: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" /></svg>,
  operational: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>,
    ...
    <aside className="h-full bg-white border-r border-slate-200 flex flex-col overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-100">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-sm shadow-emerald-100">
            <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
            </svg>
          </div>
          <div>
            <h2 className="text-slate-900 font-bold tracking-tight">Accounts</h2>
            <p className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Finance Operations</p>
          </div>
        </div>
      </div>
        ...
        {accountsNavigation
          ...
          <div key={section.category} className="space-y-1">
            <button
              ...
              className="flex items-center justify-between w-full px-3 py-1.5 text-[11px] font-bold text-slate-500 uppercase tracking-widest hover:text-slate-700 transition-colors"
            >
              {section.category}
              <svg
                className={`w-3.5 h-3.5 transition-transform duration-300 ${collapsed[section.category] ? '' : 'rotate-180'}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${collapsed[section.category] ? 'max-h-0 opacity-0' : 'max-h-[1000px] opacity-100'}`}>
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`transition-colors duration-200 ${isActive ? 'text-emerald-600' : 'text-slate-400 group-hover:text-slate-600'}`}>
                        {icons[item.icon]}
                      </span>
      ...
      <div className="shrink-0 p-4 border-t border-slate-100 bg-white/90 backdrop-blur-md">
        <div className="grid grid-cols-2 gap-3">
          <Link
            ...
            className="flex items-center justify-center gap-2 px-2 py-2.5 text-xs font-bold text-white bg-emerald-600 rounded-xl hover:bg-emerald-700 transition-all shadow-md shadow-emerald-100"
          >
