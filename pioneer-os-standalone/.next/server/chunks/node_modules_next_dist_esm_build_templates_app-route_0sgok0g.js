module.exports=[52742,e=>{"use strict";var t=e.i(747909),n=e.i(174017),a=e.i(996250),o=e.i(759756),i=e.i(561916),r=e.i(174677),s=e.i(869741),l=e.i(316795),d=e.i(487718),h=e.i(995169),c=e.i(47587),p=e.i(666012),m=e.i(570101),u=e.i(626937),y=e.i(10372),g=e.i(193695);e.i(820232);var f=e.i(600220),b=e.i(89171),v=e.i(442705),E=e.i(35344),A=e.i(469719);async function T(e,{params:t}){try{let e,n,{token:a}=await t,o=await E.prisma.employeeProposal.findUnique({where:{token:a}});if(!o)return b.NextResponse.json({error:"Not found"},{status:404});let i="BRANDING_PIONEERS"===o.entityType?{name:"Branding Pioneers",legalName:"ATZ Medappz Pvt. Ltd.",address:"750, Udyog Vihar, Third Floor, Gurgaon, Haryana"}:{name:"ATZ Medappz",legalName:"ATZ Medappz Pvt. Ltd.",address:"750, Udyog Vihar, Third Floor, Gurgaon, Haryana"},r=o.confirmedName||o.candidateName,s=new Date().toLocaleDateString("en-IN",{day:"numeric",month:"long",year:"numeric"}),l=o.bondDurationMonths||12,d=o.offeredSalary,h=o.employmentType||"FULL_TIME",c=o.probationMonths||3,p=2*d,m=3*d;return"FREELANCER"===h?(n="FREELANCER_SERVICE_AGREEMENT",e=`FREELANCER SERVICE AGREEMENT

Date: ${s}
Agreement Number: FSA-${o.id.slice(-8).toUpperCase()}

This Freelancer Service Agreement ("Agreement") is entered into as of ${s} ("Effective Date") by and between:

PARTY 1 (Company):
${i.legalName}, a company incorporated under the laws of India, operating under the trade name "${i.name}", having its registered office at ${i.address} (hereinafter referred to as the "Company").

PARTY 2 (Contractor):
${r} (hereinafter referred to as the "Contractor")
Department: ${o.department}
Engagement: ${o.position}
Agreed Fee: \u20B9${d.toLocaleString("en-IN")} per month (or as mutually agreed per deliverable)
...
13.1 The Contractor acknowledges having read and understood this Agreement, and voluntarily agrees to be bound by its terms and conditions.`):"INTERN"===h?(n="INTERNSHIP_AGREEMENT",e=`INTERNSHIP AGREEMENT

Date: ${s}
Agreement Number: IA-${o.id.slice(-8).toUpperCase()}

This Internship Agreement ("Agreement") is entered into as of ${s} ("Effective Date") by and between:

PARTY 1 (Company):
${i.legalName}, a company incorporated under the laws of India, operating under the trade name "${i.name}", having its registered office at ${i.address} (hereinafter referred to as the "Company").

PARTY 2 (Intern):
${r} (hereinafter referred to as the "Intern")
Department: ${o.department}
Designation: ${o.position}
Monthly Stipend: \u20B9${d.toLocaleString("en-IN")}
...
1.1 The internship shall commence on ${(0,v.formatDateDDMMYYYY)(o.joiningDate)} and shall continue for a period of ${l} months, unless terminated earlier in accordance with this Agreement.
...
2.1 The Company shall pay the Intern a monthly stipend of \u20B9${d.toLocaleString("en-IN")} ("Stipend").
...
16.1 The Intern acknowledges having read and understood this Agreement, and voluntarily agrees to be bound by its terms.`):(n="SERVICE_BOND",e=`SERVICE BOND AND EMPLOYMENT AGREEMENT

Date: ${s}
Agreement Number: BOND-${o.id.slice(-8).toUpperCase()}

This Service Bond and Employment Agreement ("Agreement") is entered into as of ${s} ("Effective Date") by and between:

PARTY 1 (Company):
${i.legalName}, a company incorporated under the laws of India, operating under the trade name "${i.name}", having its registered office at ${i.address} (hereinafter referred to as the "Company", which expression shall, unless repugnant to the context or meaning thereof, include its successors, affiliates, subsidiaries, and assigns).

PARTY 2 (Employee):
${r} (hereinafter referred to as the "Employee")
Department: ${o.department}
Designation: ${o.position}
Monthly CTC: \u20B9${d.toLocaleString("en-IN")}
Date of Joining: ${(0,v.formatDateDDMMYYYY)(o.joiningDate)}
...
1.1 The Employee agrees and undertakes to serve the Company for a minimum continuous period of ${l} months ("Bond Period") commencing from the date of joining, i.e., ${(0,v.formatDateDDMMYYYY)(o.joiningDate)}.
...
2.2 The estimated value of the Company's investment in the Employee's training and development over the Bond Period is \u20B9${m.toLocaleString("en-IN")}.
...
Bond Amount = (Remaining Months / ${l}) x \u20B9${p.toLocaleString("en-IN")}

Where:
- "Remaining Months" = ${l} minus the number of complete months served from the date of joining
- Maximum Bond Amount = \u20B9${p.toLocaleString("en-IN")} (2 x Monthly CTC x bond duration factor)
...
5.1 The first ${c} months from the date of joining shall constitute the probation period.
...
5.4 The Company reserves the right to extend the probation period by up to ${c} additional months if the Employee's performance does not meet the required standards.
...
(e) The restrictions and obligations in this Agreement are reasonable and necessary to protect the Company's legitimate business interests.`),b.NextResponse.json({bondContent:e,bondMonths:l,salary:d,documentType:n,employmentType:h,proposal:{candidateName:r,department:o.department,position:o.position,joiningDate:o.joiningDate.toISOString()},entity:i,alreadySigned:o.bondAccepted,signedAt:o.bondAcceptedAt?.toISOString(),signerName:o.bondSignerName})}catch(e){return console.error("Failed to fetch bond:",e),b.NextResponse.json({error:"Failed to fetch bond content"},{status:500})}}async function C(e,{params:t}){try{let{token:n}=await t,a=await e.json(),o=A.z.object({signerName:A.z.string().min(1).max(200),agreedToTerms:A.z.literal(!0),signatureData:A.z.string().max(1e4).optional(),signatureType:A.z.string().max(50).optional()}).safeParse(a);if(!o.success)return b.NextResponse.json({error:o.error.issues[0]?.message||"Invalid input"},{status:400});let i=await E.prisma.employeeProposal.findUnique({where:{token:n}});if(!i)return b.NextResponse.json({error:"Not found"},{status:404});if(i.expiresAt&&i.expiresAt<new Date)return b.NextResponse.json({error:"This onboarding link has expired"},{status:410});if(!i.ndaAccepted)return b.NextResponse.json({error:"Please sign the NDA first"},{status:400});if(i.bondAccepted)return b.NextResponse.json({success:!0,message:"Bond already signed"});return await E.prisma.employeeProposal.update({where:{token:n},data:{bondAccepted:!0,bondAcceptedAt:new Date,bondSignerName:a.signerName,bondSignatureData:a.signatureData||null,bondSignatureType:a.signatureType||"type",status:"BOND_SIGNED",currentStep:5}}),b.NextResponse.json({success:!0,currentStep:5})}catch(e){return console.error("Failed to sign bond:",e),b.NextResponse.json({error:"Failed to sign bond"},{status:500})}}e.s(["GET",0,T,"POST",0,C],359822);var N=e.i(359822);let w=new t.AppRouteRouteModule({definition:{kind:n.RouteKind.APP_ROUTE,page:"/api/employee-onboarding/[token]/bond/route",pathname:"/api/employee-onboarding/[token]/bond",filename:"route",bundlePath:""},distDir:".next",relativeProjectDir:"",resolvedPagePath:"[project]/src/app/api/employee-onboarding/[token]/bond/route.ts",nextConfigOutput:"standalone",userland:N}),{workAsyncStorage:I,workUnitAsyncStorage:R,serverHooks:D}=w;async function S(e,t,a){a.requestMeta&&(0,o.setRequestMeta)(e,a.requestMeta),w.isDev&&(0,o.addRequestMeta)(e,"devRequestTimingInternalsEnd",process.hrtime.bigint());let b="/api/employee-onboarding/[token]/bond/route";b=b.replace(/\/index$/,"")||"/";let v=await w.prepare(e,t,{srcPage:b,multiZoneDraftMode:!1});if(!v)return t.statusCode=400,t.end("Bad Request"),null==a.waitUntil||a.waitUntil.call(a,Promise.resolve()),null;let{buildId:E,params:A,nextConfig:T,parsedUrl:C,isDraftMode:N,prerenderManifest:I,routerServerContext:R,isOnDemandRevalidate:D,revalidateOnlyGenerated:S,resolvedPathname:O,clientReferenceManifest:P,serverActionsManifest:x}=v,k=(0,s.normalizeAppPath)(b),M=!!(I.dynamicRoutes[k]||I.routes[O]),L=async()=>((null==R?void 0:R.render404)?await R.render404(e,t,C,!1):t.end("This page could not be found"),null);if(M&&!N){let e=!!I.routes[O],t=I.dynamicRoutes[k];if(t&&!1===t.fallback&&!e){if(T.adapterPath)return await L();throw new g.NoFallbackError}}let $=null;!M||w.isDev||N||($="/index"===($=O)?"/":$);let j=!0===w.isDev||!M,U=M&&!j;x&&P&&(0,r.setManifestsSingleton)({page:b,clientReferenceManifest:P,serverActionsManifest:x});let B=e.method||"GET",G=(0,i.getTracer)(),H=G.getActiveScopeSpan(),F=!!(null==R?void 0:R.isWrappedByNextServer),q=!!(0,o.getRequestMeta)(e,"minimalMode"),W=(0,o.getRequestMeta)(e,"incrementalCache")||await w.getIncrementalCache(e,T,I,q);null==W||W.resetRequestCache(),globalThis.__incrementalCache=W;let Y={params:A,previewProps:I.preview,renderOpts:{experimental:{authInterrupts:!!T.experimental.authInterrupts},cacheComponents:!!T.cacheComponents,supportsDynamicResponse:j,incrementalCache:W,cacheLifeProfiles:T.cacheLife,waitUntil:a.waitUntil,onClose:e=>{t.on("close",e)},onAfterTaskError:void 0,onInstrumentationRequestError:(t,n,a,o)=>w.onRequestError(e,t,a,o,R)},sharedContext:{buildId:E}},_=new l.NodeNextRequest(e),V=new l.NodeNextResponse(t),z=d.NextRequestAdapter.fromNodeNextRequest(_,(0,d.signalFromNodeResponse)(t));try{let o,r=async e=>w.handle(z,Y).finally(()=>{if(!e)return;e.setAttributes({"http.status_code":t.statusCode,"next.rsc":!1});let n=G.getRootSpanAttributes();if(!n)return;if(n.get("next.span_type")!==h.BaseServerSpan.handleRequest)return void console.warn(`Unexpected root span type '${n.get("next.span_type")}'. Please report this Next.js issue https://github.com/vercel/next.js`);let a=n.get("next.route");if(a){let t=`${B} ${a}`;e.setAttributes({"next.route":a,"http.route":a,"next.span_name":t}),e.updateName(t),o&&o!==e&&(o.setAttribute("http.route",a),o.updateName(t))}else e.updateName(`${B} ${b}`)}),s=async o=>{var i,s;let l=async({previousCacheEntry:n})=>{try{if(!q&&D&&S&&!n)return t.statusCode=404,t.setHeader("x-nextjs-cache","REVALIDATED"),t.end("This page could not be found"),null;let i=await r(o);e.fetchMetrics=Y.renderOpts.fetchMetrics;let s=Y.renderOpts.pendingWaitUntil;s&&a.waitUntil&&(a.waitUntil(s),s=void 0);let l=Y.renderOpts.collectedTags;if(!M)return await (0,p.sendResponse)(_,V,i,Y.renderOpts.pendingWaitUntil),null;{let e=await i.blob(),t=(0,m.toNodeOutgoingHttpHeaders)(i.headers);l&&(t[y.NEXT_CACHE_TAGS_HEADER]=l),!t["content-type"]&&e.type&&(t["content-type"]=e.type);let n=void 0!==Y.renderOpts.collectedRevalidate&&!(Y.renderOpts.collectedRevalidate>=y.INFINITE_CACHE)&&Y.renderOpts.collectedRevalidate,a=void 0===Y.renderOpts.collectedExpire||Y.renderOpts.collectedExpire>=y.INFINITE_CACHE?void 0:Y.renderOpts.collectedExpire;return{value:{kind:f.CachedRouteKind.APP_ROUTE,status:i.status,body:Buffer.from(await e.arrayBuffer()),headers:t},cacheControl:{revalidate:n,expire:a}}}}catch(t){throw(null==n?void 0:n.isStale)&&await w.onRequestError(e,t,{routerKind:"App Router",routePath:b,routeType:"route",revalidateReason:(0,c.getRevalidateReason)({isStaticGeneration:U,isOnDemandRevalidate:D})},!1,R),t}},d=await w.handleResponse({req:e,nextConfig:T,cacheKey:$,routeKind:n.RouteKind.APP_ROUTE,isFallback:!1,prerenderManifest:I,isRoutePPREnabled:!1,isOnDemandRevalidate:D,revalidateOnlyGenerated:S,responseGenerator:l,waitUntil:a.waitUntil,isMinimalMode:q});if(!M)return null;if((null==d||null==(i=d.value)?void 0:i.kind)!==f.CachedRouteKind.APP_ROUTE)throw Object.defineProperty(Error(`Invariant: app-route received invalid cache entry ${null==d||null==(s=d.value)?void 0:s.kind}`),"__NEXT_ERROR_CODE",{value:"E701",enumerable:!1,configurable:!0});q||t.setHeader("x-nextjs-cache",D?"REVALIDATED":d.isMiss?"MISS":d.isStale?"STALE":"HIT"),N&&t.setHeader("Cache-Control","private, no-cache, no-store, max-age=0, must-revalidate");let h=(0,m.fromNodeOutgoingHttpHeaders)(d.value.headers);return q&&M||h.delete(y.NEXT_CACHE_TAGS_HEADER),!d.cacheControl||t.getHeader("Cache-Control")||h.get("Cache-Control")||h.set("Cache-Control",(0,u.getCacheControlHeader)(d.cacheControl)),await (0,p.sendResponse)(_,V,new Response(d.value.body,{headers:h,status:d.value.status||200})),null};F&&H?await s(H):(o=G.getActiveScopeSpan(),await G.withPropagatedContext(e.headers,()=>G.trace(h.BaseServerSpan.handleRequest,{spanName:`${B} ${b}`,kind:i.SpanKind.SERVER,attributes:{"http.method":B,"http.target":e.url}},s),void 0,!F))}catch(t){if(t instanceof g.NoFallbackError||await w.onRequestError(e,t,{routerKind:"App Router",routePath:k,routeType:"route",revalidateReason:(0,c.getRevalidateReason)({isStaticGeneration:U,isOnDemandRevalidate:D})},!1,R),M)throw t;return await (0,p.sendResponse)(_,V,new Response(null,{status:500})),null}}e.s(["handler",0,S,"patchFetch",0,function(){return(0,a.patchFetch)({workAsyncStorage:I,workUnitAsyncStorage:R})},"routeModule",0,w,"serverHooks",0,D,"workAsyncStorage",0,I,"workUnitAsyncStorage",0,R],52742)}];

//# sourceMappingURL=node_modules_next_dist_esm_build_templates_app-route_0sgok0g.js.map